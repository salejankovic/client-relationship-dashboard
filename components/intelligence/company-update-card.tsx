...
import { ExternalLink, Lightbulb, X, Sparkles, Building2 } from "lucide-react"
...
export function CompanyUpdateCard({
  item,
  prospect,
  onDismiss,
  onUseInFollowUp,
}: CompanyUpdateCardProps) {
  ...
  const isFunding = item.sourceType === 'funding' || item.intelligenceType === 'funding'
  const badgeClass = isFunding
    ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900 dark:text-yellow-300'
    : 'bg-cyan-100 text-cyan-700 dark:bg-cyan-900 dark:text-cyan-300'
  const avatarClass = isFunding ? 'bg-yellow-600' : 'bg-cyan-600'
  const badgeIcon = isFunding ? '💰' : ''
  const badgeText = isFunding ? 'FUNDING' : 'COMPANY UPDATE'
      ...
      <CardContent className="p-6">
        ...
        <div className="flex items-start justify-between mb-4">
          <span className={`text-xs font-semibold px-2 py-1 rounded ${badgeClass}`}>
            {badgeIcon || <Building2 className="h-3 w-3 inline mr-1" />}
            {badgeText}
          </span>
        ...
        <div className="flex items-start gap-3 mb-4">
          <div className={`w-10 h-10 rounded-full ${avatarClass} flex items-center justify-center text-white font-medium text-sm`}>
            {initials}
          </div>
          <div>
            <div className="flex items-center gap-2">
            ...
            <p className="text-sm text-muted-foreground">
              Source: {item.sourceName || 'LinkedIn'}
            </p>
          </div>
        </div>
